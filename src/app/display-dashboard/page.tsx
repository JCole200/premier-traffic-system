...
export default async function DisplayDashboard({ searchParams }: { searchParams: Promise<{ date?: string; endDate?: string }> }) {
            ...
            <main className="grid-dashboard">
                ...
                <section style={{ padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
                    <div className="glass-panel" style={{ padding: '3rem', borderRadius: '24px', textAlign: 'center', maxWidth: '500px' }}>
                        ...
                        <h1 style={{ marginBottom: '1rem', fontSize: '2rem', fontWeight: 700 }}>Display Impressions</h1>
                        <p style={{ color: 'var(--text-muted)', marginBottom: '2rem', lineHeight: 1.6 }}>
                            Choose your campaign start date to see inventory capacity across all Premier portfolio websites.
                        </p>
                        <DateSelectionWizard pathname="/display-dashboard" type="DISPLAY" />
                    </div>
                </section>
            </main>
        );
    ...
    const displayItems = allItems.filter((i: any) => i.type === 'DISPLAY');

    return (
        <main className="grid-dashboard">
            <Sidebar />
            <section style={{ padding: '2rem' }}>
                <DashboardHeader initialDate={selectedDate} title="Display Dashboard" />

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
                    {await Promise.all(displayItems.map(async (item: any) => {
                        const avail = await checkAvailability('DISPLAY', selectedDate, end, item.id);
                        const usedPct = ((item.totalCapacity - avail) / item.totalCapacity) * 100;

                        return (
                            <div key={item.id} className="glass-panel" style={{ padding: '2rem', borderRadius: '16px', borderTop: `4px solid var(--accent-cyan)` }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1.5rem' }}>
                                    ...
                                    <Link href={`/booking?type=DISPLAY&target=${item.id}&start=${selectedDate}`}>
                                        <button className="btn-primary" style={{ fontSize: '0.8rem', padding: '0.5rem 1rem', background: 'var(--accent-cyan)' }}>
                                            Book This
                                        </button>
                                    </Link>
                                </div>
