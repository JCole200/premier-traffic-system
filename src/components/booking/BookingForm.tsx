                                ...
                                >
                                    <div>
                                        ...
                                        <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', lineHeight: '1.6' }}>
                                            ...
                                            {booking.bookerName && <div><strong>Booked by:</strong> {booking.bookerName}</div>}
                                            {booking.contractNumber && <div><strong>Contract:</strong> {booking.contractNumber}</div>}
                                            <div><strong>Status:</strong> <span style={{ color: 'var(--success)' }}>{booking.status}</span></div>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', gap: '0.5rem', flexDirection: 'column' }}>
                                        <button
                                            type="button"
                                            onClick={() => loadBookingForEdit(booking)}
                                            style={{
                                                padding: '0.5rem 1rem',
            ...
            {(!isAdmin || showNewForm) && (
                <form onSubmit={handleSubmit} className="glass-panel" style={{ padding: '2.5rem', borderRadius: '24px', maxWidth: '800px', margin: '0 auto' }}>
                    ...
                    <h2 style={{ fontSize: '1.8rem', marginBottom: '2rem', color: 'var(--text-main)' }}>
                        {editingBookingId ? 'Edit Booking' : 'New Booking Request'}
                    </h2>
                    ...
                    <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
                        ...
                        >
                            {isSubmitting ? (editingBookingId ? 'Updating...' : 'Booking...') : (editingBookingId ? 'Update Booking' : 'Confirm Booking')}
                        </button>
                    </div>
                </form>
            )}
